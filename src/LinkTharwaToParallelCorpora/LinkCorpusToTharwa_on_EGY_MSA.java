...
This java class aims to extract example sentences for EGY-MSA pairs in Tharwa. ENG equivalents are incorporated to consider different
 senses of words. As the result, each EGY word in Tharwa is connected to the sentences from parallel data which have that partcular EGY
...
public class LinkCorpusToTharwa_on_EGY_MSA {
    ...
    public static HashMap<Integer, HashMap<Integer, HashMap<Integer, Set<Integer>>>> ConvertTharwaToDic(String tharwaPath, boolean AlefYaNormalized, boolean diacritized) throws IOException
    {
        System.out.println("***************************");
        ...
        while ((LineToRead =tharwaReader.readLine()) != null) {
            ...
            if (AlefYaNormalized == true) {
                if (diacritized == false) {
                    DA_word = getSafeBW(normalizeAlefYa(undiacratize(DA_word)));
                    MSA_word = getSafeBW(normalizeAlefYa(undiacratize(MSA_word)));

                } else {
                    DA_word = getSafeBW(normalizeAlefYa(DA_word));
                    MSA_word = getSafeBW(normalizeAlefYa(MSA_word));

                }
            } else {
                if (diacritized == false) {
                    DA_word = getSafeBW(undiacratize(DA_word));
                    MSA_word = getSafeBW(undiacratize(MSA_word));

                } else {
                    DA_word = getSafeBW(DA_word);
                    MSA_word = getSafeBW(MSA_word);
    ...
    public static HashMap<Integer,HashMap<Integer, Set<ArrayList<Integer>>>> build_Ar_EnAlignDic(String Ar_corpus, String En_corpus,
                                                                                                 String Allign_file, String parallelDataType, boolean AlefYaNormalized, boolean diacritized) throws IOException
    {
        ...
        while (((Ar_line_to_read = Ar_data_reader.readLine()) != null))
        {
            ...
            for (int i=0;i<Allign_words.length;i++) {
                ...
                if (AlefYaNormalized==true)
                {
                    if (diacritized==false)
                    {
                        ArabicLemma = getSafeBW(normalizeAlefYa_in_SafeBW(undiacratize(ArabicLemma)));
                    }
                    else {
                        ArabicLemma = getSafeBW(normalizeAlefYa_in_SafeBW(ArabicLemma));
                    }
                }
                else
                {
                    if (diacritized== false)
                    {
                        ArabicLemma= getSafeBW(undiacratize(ArabicLemma));
                    }
                    else
                    {
                        ArabicLemma= getSafeBW(ArabicLemma);
                    }
                }
